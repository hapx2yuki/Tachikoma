...
## 注意
- 2026-09-03 に「LD-220MG→DS3218 変換クランプ」図面が検討されたが、外形が既存ポケット (幅 20.6) に入らず不採用。
  **正攻法は実測 → config 更新 → 再生成** (この実測がその入口)
...
for t in check_leg_assembly check_screw_bosses sim_gait check_arm check_eye check_audio \\
         check_camera check_shin_arm_leg check_head_pod_clearance check_pod_neck_strength check_urdf; do
...
issue("PR-04", "PR-04 [印刷] PETG_Walk_3_Tibia — tibia_link ×2 + _m ×2 立て (151g / 9.3h, A4 PETG)",
...
2026-08-21b の膝ディスク分離修正 (ガード r23) が入った 3mf であること。
...
issue("L-02", "L-02 [ゲート] 片脚ベンチ試験 — 1.2kg リフト (6V) / スイープ / 保持電流 <1.5A",
      parent="E4", milestone="M1 片脚 Go/No-Go",
...
- [ ] 足先に **1.2kg の錘** (総重量 3.0kg × 最悪脚負担 40%) を股ピッチ・膝で持ち上げられる
- [ ] 保持時の電流を実測し **< 1.5A / サーボ** (目安)
...
設計値見直し → `config.py` → `build_all.py` → 再テスト印刷 のループ。設計側の想定トルクは股ピッチ 8.9kg·cm (定格 20 の 45%)。
...
- [ ] 歩行中の 6V レール電流・バッテリー電圧を記録 (UBEC HENGE 8A の入力下限 7V によるブラウンアウトが出ないか)
...
4. **固定爪**: claw_mount の背面を forearm の手首面へ接着 (瞬着/エポキシ) → 爪ハブ `Arm_Left_Claw_Grey` の平坦近位面を claw_mount 前面へ突き合わせ接着 (軽く均す) → `Arm_Left_Finger_Black` ×3 をハブの 3 ペグへ圧入+接着 → `Arm_Left_FingerTip_Grey` ×3 を指根元へ接着
...
issue("A-03", "A-03 [組立] 肩ヨーサーボ ×2 をシャーシへ + 両腕を吊り下げ + 配線 + ベンチ確認 (TUCK/READY/REACH/WAVE)",
      ...
      blocked_by=["A-01", "A-02", "L-09", "EL-06"],
      body=f"""
...
3. 超過が大きい (>3.3kg) 場合は股ピッチトルク余裕 (45%) を再計算 → `sim_gait.py` の総重量前提を更新
