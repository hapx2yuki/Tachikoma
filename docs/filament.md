...
**追加注文には [最新の購入・残量確認一覧](additional-purchases.md) を使う。** Amazon 履歴では PLA 赤・青・黒・灰と TPU 黒が各 1kg 配達済み。過去の白/PETG 装着記録とは別の根拠で、現在の残量は未確認。下表の推奨巻数をそのまま追加注文しない。

**2026-09-05:** 下表は8月22日版の概算を保存したもの。音声・腕・Cabinの内部修正と
不使用Head_Plate/Bottom_Capの計上修正を反映した[現行概算](audits/20260905-round2/manufacturing.md)を参照。購入済み在庫を先に照合する。
足のTPUは先行接地を満たしておらず、2gを完成版の必要量としない。
