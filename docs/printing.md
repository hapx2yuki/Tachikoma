...
**今回追加で印刷する候補・再使用・保留は [印刷優先構成の現在地](print-first.md) と [部品別の全一覧](additional-printing.md) を参照。** 下表は機体1台の必要総数であり、手持ちを差し引いた追加数ではない。手動編集済みの3MFは部品を除いた状態を含むため、旧プレート表から実収録数を推測しない。
...
- 骨格はレイヤー 0.2mm / ノズル 0.4mm。**PETG 指定** (PLA は夏場の車内・
 直射日光でクリープする)
- **サーボ実測 → `hardware/src/config.py` 更新 → 再生成・部品ごとの干渉確認を先に行う**。
 既印刷品を確認し、不足または再使用できない coxa_bracket / femur_link / tibia_link / leg_foot_bored を
 試験脚用に各1個だけ用意する。現行foot_padの追加印刷はRV-06で支持形状を確定してから判断し、
 片脚の荷重・はめあい試験に合格してから残りを印刷する (assembly.md の Go/No-Go 手順)。
...
- **旧接地補正の記録 (2026-07-29。TPU支持の証明には使わない)**: leg_foot_bored+foot_pad は tibia の物理
 ...
 `TIBIA_LEN_GAIT` (= 135 + `FOOT_GROUND_OFFSET`=20.98 = 155.98mm,
