...
| 配線の出口 | 底側の端面 (箱枠の -X 端に逃がし穴あり) | **出力軸側端面の上面直下** (既存の逃がしが使えない) |
...
- ケース上面の Y は DS3218 と同じ 11.0 (ホーンアーム上面 17.5) に合わせてある。
  **LD-220MG のケース高さ H と、上面〜ホーンアーム上面の距離 HORN_TOP を
  ...
  ホーンの高さがそのままずれる)。
- 出力軸から遠い側の底板角は 6×3 mm の面取り。後脚の股ピッチカップが
  ポッド側ヨー 22° でバッテリーパックに 2.4 mm 食い込むのを避けるため。
...
| `coxa_bracket_ld220(_m).stl` `femur_link_ld220(_m).stl` | 任意 | 配線ノッチ+溝を焼き込んだ箱枠。再印刷する場合のみ |

印刷: PETG、壁 4・インフィル 40%。**フランジ面 (取付面) を下**にして印刷
(底板が 20.4 mm のブリッジになるが PETG で問題ない範囲)。

## 組み方 (脚: 股ピッチ / 膝)

1. 箱枠の **+Y 面 (ホーン側) の出力軸寄りの端** に配線用の切り欠きを入れる:
   ポケット端から **5 mm 幅 × 7 mm 深のノッチ** (2 つのタブ穴の間)、そこから
   箱枠の端まで **5 mm 幅 × 2 mm 深の溝**。溝に沈めないと関節相手の円板
   (隙間 1.8 mm) に配線が挟まれる。リューターかホビーのこ + ヤスリで可。
   再印刷するなら `*_ld220.stl` に同じ形が入っている。
2. LD-220MG を中立 (1500µs) にして、配線を先にノッチへ通しながら +Y 側から
   ポケットへ入れる。
3. 箱枠の -Y 面からカップを被せ、**M3×10 タッピング ×4** を既存のタブ穴へ。
   ビス頭はカップのフランジ上に座る (端壁に逃がしあり)。
4. ホーンは DS3218 と同じ位置 (tibia / femur の円板ポケット) に共締め。
...
- [1] カップ ∩ 箱枠 = 0、カップ ∩ ケース (はめあい込み) = 0、タブ穴 4 本が同軸
...
- `LD220`: H / HORN_TOP / REAR_BOSS_D,H / WIRE_BELOW_TOP / BOT_HOLES は
