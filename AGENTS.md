...
ユーザー指定により、Astra は設計・オーケストレーション・レビューに徹し、実装・調査実務・
検証・公開の作業は Luna Max Fast をサブエージェントとして担当させる。Fast の独立設定が
利用できない環境では、利用可能な優先枠を使い、別モデルへ無断で差し替えない。利用不能時は
その制約を作業記録へ明記する。
...
1. **購入済み部品を優先する** — 2026-09-05のユーザー指示により、追加購入を避けるための
   可視部の形状・配置変更、外付け印刷部品、装飾の一時保管を許可。元STLと既印刷品は保持し、
   新しい構成は `docs/print-first.md` に再使用品・追加印刷・実機未確認事項を明記する。
   過去の「可視部変更には再確認」という制約は、この目的の変更には適用しない。
