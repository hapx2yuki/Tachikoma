...
# ---------------------------------------------------------------- LD-220MG 用カップ (2026-09-05)
# 実際に届いた脚サーボは Hiwonder LD-220MG (タブ無しケース 40×20, 上下面に
# ねじ穴, 底面に補助軸ボス, 配線は出力軸側端面の上面直下から出る)。
# STD (DS3218 想定) の箱枠 / シャーシ開口にはケースがそのまま通るので、
# ケース底面側から被せるカップ (ld220_cup_*) で位置決め・固定する。
# [要実測] 印は写真 (userInput/IMG_1899〜1910) からの読み取り値。組む前に
# ノギスで確認し、違えば書き換えて make_ld220_cup.py を再実行すること。
LD220 = dict(
    L=40.0, W=20.0,             # ケース外形 (写真の実測 40.0 / 20.0)
    H=40.5,                     # [要実測] 底面〜上面 (出力軸ボス除く) の高さ
    HORN_TOP=6.5,               # [要実測] 上面〜ホーンアーム上面 (STD と同じ 6.5 を仮定。
                                #   これが違うと関節の噛み合い高さがずれるので必ず実測)
    REAR_BOSS_D=12.0,           # [要実測] 底面の補助軸ボス外径
    REAR_BOSS_H=2.0,            # [要実測] 同 高さ
    WIRE_END="+x",              # 配線の出る端面 (出力軸に近い側 = +x)。底面寄りから出る
    WIRE_ABOVE_BOT=4.0,         # [要実測] 底面から配線中心までの距離 (写真では 3〜5mm)
    BOT_HOLES=None,             # 底面ねじ穴 (x ピッチ, z ピッチ, 径) [要実測]。None なら穴を開けない
)
LD220_CUP = dict(
    WALL=2.5,                   # 側壁 (幅方向)。箱枠幅 26.2 内に収める: 20.4+2*2.5 = 25.4
    END_WALL=1.8,               # 端壁 (長手方向)。タブ穴のビス頭と干渉しない厚さ
    FLOOR=3.0,                  # 底板
    FLANGE=3.0,                 # 箱枠 -Y 面 / シャーシボス上面に載るフランジ
    SCREW_D=3.4,                # M3 タッピングの通し穴 (箱枠側の下穴 φ2.8 に噛ませる)
    HEAD_D=6.6,                 # ビス頭の逃がし径
    CLEAR=0.20,                 # ケースとのはめあい
    OPEN_END_SETBACK=0.8,       # 出力軸から遠い側 (-x) は端壁なし。底板/側壁はケース端の 0.8mm 手前で終わる
    FLOOR_SETBACK=8.0,          # 開放端からこの距離まで底板/側壁下部を省く (LIM_YAW_POD=30° でバッテリーパックを躱す)
    CHAMFER_X=3.0, CHAMFER_Z=3.0,   # 底板の開放端側の角の面取り
    WIRE_HOLE_W=9.0, WIRE_HOLE_H=7.0,  # +x 端壁の配線穴 (3 ピンコネクタごと通る)
    SKIRT_T=1.5, SKIRT_H=3.0, SKIRT_HALF_W=6.5,  # 箱枠端面にはまるスカート (厚/高さ/半幅)
)
