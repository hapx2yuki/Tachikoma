...
class AsyncWebSocketClient {
 uint32_t id_;
 public:bool closed=false;AsyncWebSocketClient(uint32_t id):id_(id){}
 uint32_t id(){return id_;}void close(){closed=true;}void text(const char*){}
};
class AsyncWebSocket {
 public:
 std::vector<uint8_t> binary;std::vector<std::string> messages;
 AsyncWebSocket(const char* path=""){}
 template<class T>void onEvent(T){}void cleanupClients(){}int count(){return 1;}
 void textAll(const String& s){messages.push_back(s);}
 void textAll(const char* s){messages.push_back(s);}
 void binaryAll(uint8_t* p,size_t n){binary.insert(binary.end(),p,p+n);}
};
...
class AsyncWebServer {
 ...
 template<class T>void on(const char* path,int method,T callback){if(method==HTTP_GET)routes[path]=callback;}
 void begin(){}
};
