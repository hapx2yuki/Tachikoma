...
// 印刷優先構成は、歩容候補を比較した体高へ固定する。これは採用済みや
// 実機で検証済みという意味ではない。生成ヘッダーが hardware/src/config.py
// の PRINT_FIRST_GAIT を唯一の値として持つ。
constexpr float TK_CONTROL_BODY_H_DEFAULT = PRINT_FIRST_BODY_H;
constexpr float TK_CONTROL_BODY_H_MIN = PRINT_FIRST_BODY_H;
constexpr float TK_CONTROL_BODY_H_MAX = PRINT_FIRST_BODY_H;
#else
constexpr float TK_CONTROL_BODY_H_DEFAULT = BODY_H_DEF;
constexpr float TK_CONTROL_BODY_H_MIN = BODY_H_MIN;
constexpr float TK_CONTROL_BODY_H_MAX = BODY_H_MAX;
#endif

struct ControlState {
  float vx = 0, vy = 0, wz = 0, h = TK_CONTROL_BODY_H_DEFAULT;
...
inline bool updateControlFromJson(JsonDocument& doc, ControlState& state, uint32_t now) {
  ...
  if (!doc["h"].isNull()) state.h = constrain(doc["h"].as<float>(), TK_CONTROL_BODY_H_MIN, TK_CONTROL_BODY_H_MAX);
